...
SPACE_COMMA_SEPARATOR = re.compile(r"\s*[\s,]\s*")
...
def parseListOfPoints(s):
    ...
    ws_nums = SPACE_COMMA_SEPARATOR.split(s.strip())
    nums = []
...
def properlySizeDoc(docElement, options):
    ...
    vbSep = SPACE_COMMA_SEPARATOR.split(docElement.getAttribute('viewBox'))
...
def _attribute_sort_key_function(attribute):
    name = attribute.name
    order_value = KNOWN_ATTRS_ORDER_BY_NAME[attribute.name]
    return order_value, name


def attributes_ordered_for_output(elem):
    ...
    # - it would be many times faster to use `attr.values()` but sadly that
    #   is an "experimental" interface.
    return sorted((attr.item(i) for i in range(attr.length)),
...
def scourString(in_string, options=None):
    ...
    if viewBox:
        lengths = SPACE_COMMA_SEPARATOR.split(viewBox)
